...
client.on("interactionCreate", interaction => {
  if (interaction.customId === "fast") {
    const queue = client.distube.getQueue(interaction);
       if (!queue) return interaction.reply(`Không có bài hát nào trong danh sách.`)
    let data = db.fetch(`music_${interaction.guild.id}`)
    if (!data) return interaction.reply({content: "Có lỗi", ephemeral: true})
    queue.filters.add("nightcore")
interaction.reply({content: "Tốc độ trực tuyến!", ephemeral: true})
  }
...
interaction.reply({content: "Chế độ chậm!", ephemeral: true})
  }
...
interaction.reply({content: "Tăng bass!", ephemeral: true})
  }
...
if (interaction.customId === "dur") {
  ...
  if (interaction.user.id !== usır) return interaction.reply({content: "Chỉ người đã viết lệnh mới có thể sử dụng nút này.", ephemeral: true})
...
const embed = new Discord.EmbedBuilder()
.addFields({name: "Tiêu đề", value: `${baslik}`, inline: true})
.addFields({name: "Tác giả", value: `${author}`, inline: true})
.addFields({name: "Thời gian", value: `${sure}`, inline: true})
.addFields({name: "Lượt xem", value: `${izlenme}`, inline: true})
...
if (interaction.customId === "skip") {
  ...
  let data = db.fetch(`music_${interaction.guild.id}`)
  if (!data) return interaction.reply({content: "Có lỗi", ephemeral: true})
    if (queue.songs.length === 1) return interaction.reply("Không tìm thấy bài hát nào trong hàng đợi!")
  let usır = data.user
  let string = data.string
  if (interaction.user.id !== usır) return interaction.reply({content: "Chỉ người đã viết lệnh mới có thể sử dụng nút này.", ephemeral: true})
const baslik = data.başlık
